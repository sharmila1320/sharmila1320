<div align="center">

   <p align="center">
      <a href="mailto:rapetisug22@ece.nits.ac.in" target="_blank">
        <img src="https://img.shields.io/badge/Sharmila-EE0000?style=for-the-badge&logo=gmail&logoColor=white"/>
      </a>
      <a href="https://discord.com/users/yourdiscordid" target="_blank">
        <img src="https://img.shields.io/badge/Sharmila%20-%237289DA.svg?&style=for-the-badge&logo=discord&logoColor=white"/>
      </a>
   </p>

   <p align="center">
      <a href="https://stackoverflow.com/users/your-id" target="_blank">
        <img src="https://img.shields.io/badge/Sharmila%20Rapeti%20-EC5800?style=for-the-badge&logo=stack-overflow&logoColor=white"/>
      </a>
      <a href="https://www.linkedin.com/in/sharmilarapeti" target="_blank">
        <img src="https://img.shields.io/badge/Sharmila%20Rapeti-0077B5?style=for-the-badge&logo=linkedin&logoColor=white"/>
      </a>
   </p>
</div>

<br>

<div align="center">
  <img src="https://github.com/sharmila1320/sharmila1320/blob/main/waving_gif.gif" width="300px">
</div>



<br>

<h2 align="center"> 👁️‍🗨️ About Me 👁️‍🗨️ </h2>


Profile ver: 星々に触れる者 (One who dreams in constellations)
---------------------------------------------------------------
Username: Sharmila Rapeti<br>
Who-am-I: Hybrid SDE + AI/ML Engineer • ECE @ NIT Silchar<br>
Role: Student • Builder • Researcher • Creative Technologist<br>
OS: Windows 11<br>
Interests: Deep Learning, Full-stack apps, CV/NLP, Space Tech<br>
Pronouns: She / Her<br>
Location: India<br>

Languages: C++, Python, JavaScript, SQL<br>
Current Learning: FastAPI, Next.js, LLMs, Transformers, CV, 
                  ML/DL workflows, advanced backend systems<br>

FavSong: Diet Pepsi - Addison Rae<br>
FavConcept: Gen-AI & LLMs<br>
Hobbies: Reading, designing clothes, yoga, anime aesthetics,
         building intelligent systems with creativity<br>
<br><br><br>

<h2 align="center">  Experience </h2>
🔹 Summer Research Intern — SN BOSE – NIT Silchar (May–July 2025)<br>
MARLINet: Hybrid CNN–GNN Underwater Image Enhancement Model<br>

Designed & trained a CNN-GNN hybrid architecture (UIEB dataset).<br>

Achieved 16.91 dB PSNR & 0.6759 SSIM, outperforming baseline CNN models.<br>

Built patch–graph construction pipeline & complete training workflow.<br>

Wrote and submitted the research paper to a Springer international conference.<br>



<h2 align="center">  Projects </h2>
 CampusGPT — AI Campus Assistant<br>
Built a full-stack campus AI system with domain login & role-based features.<br>

Chat assistant that learns from user responses & escalates unknown queries.<br>

Tech Stack: React, TypeScript, Tailwind, Node.js, Express, OpenAI API<br>



 QuickCV — MERN Resume Builder<br>
4 customizable templates, autosave, LinkedIn import.<br>

Designed secure MongoDB schemas + Express backend (auth + error handling).<br>

Tech Stack: React, CSS, Node.js, Express.js, MongoDB<br>



 Spam Mail Detection — ML<br>
Implemented Naive Bayes, SVM, Decision Trees for email classification.<br>

Achieved 99.6% accuracy with optimized preprocessing.<br>

Used NLP vectorization + scikit-learn evaluation pipelines.<br>


<br> <h2 align="center">  Skills </h2> <div align="center">
Programming<br>
C++, Python, JavaScript, SQL<br>

Development<br>
DSA, OOP, DBMS, OS, REST APIs, Git/GitHub, Linux, Unit Testing (Basics)<br>

Web / Backend<br>
Node.js, Express.js, FastAPI, Next.js, React.js, HTML/CSS<br>
MySQL, PostgreSQL, MongoDB, ChromaDB<br>

Machine Learning<br>
Supervised & Unsupervised ML, Feature Engineering, Model Evaluation<br>

Deep Learning<br>
CNNs, RNNs, Transformers (Basics), Computer Vision, NLP<br>

Frameworks / Tools<br>
TensorFlow, Keras, Scikit-Learn, NLTK, OpenCV, Pandas, NumPy<br>

</div> <br> <div align="center"> <a href="https://skillicons.dev"> <img style="margin: 10px" src="https://skillicons.dev/icons?i=c,cpp,py,js,react,nodejs,nextjs,html,css,git,github,linux,mysql,postgresql,mongodb,fastapi,tensorflow,sklearn,opencv&perline=8" /> </a> </div>
<h2 align="center"> 🏆 Achievements </h2>
Active open-source contributor at GSSoC'25<br>

Solved 500+ problems on LeetCode, GFG, Codeforces<br>

Participated in Tek-Qubit : qualified phase2, SheCodes, Flipkart Grid '24/'25, Walmart Sparkathon, Adobe India, CodeHer<br>


<h2 align="center">  Positions of Responsibility </h2>
Head of Publications, Illuminits (English Literature Club)<br>

Social Media Manager, AAVEG<br>

Junior Event Coordinator, E-CELL<br>


<h2 align="center">  GitHub Stats </h2> <div align="center"> <a href="https://github-readme-stats.vercel.app"> <img width="49%" alt="Stats" src="https://github-readme-stats.vercel.app/api?username=sharmila1320&count_private=true&theme=neon&show_icons=true&rank_icon=github&hide_border=false"> </a>
<a href="https://github-readme-streak-stats.herokuapp.com">
    <img width="49%" alt="Streak Stats" 
         src="https://github-readme-streak-stats.herokuapp.com/?user=sharmila1320&theme=neon&hide_border=false&date_format=%5BY%20%5DM%20j">
</a>


<a href="https://github.com/ashutosh00710/github-readme-activity-graph">
    <img width="120%" alt="Activity Graph" 
         src="https://github-readme-activity-graph.vercel.app/graph?username=sharmila1320&theme=redical">
</a>

<a href="https://github.com/vn7n24fzkq/github-profile-summary-cards">
    <img width="120%" alt="GraphStats" 
         src="http://github-profile-summary-cards.vercel.app/api/cards/profile-details?username=sharmila1320&theme=2077">
</a>
</div>
<br><br>

<div align="center"> <h2>Thank you for being here 💫</h2> <img src="https://i.pinimg.com/originals/8f/71/e5/8f71e53b785d5ac6c2dfd5e88c9acddd.gif" width="450"/> </div> 

